...
import cn.facebook.excelUtil.DataFormatUtilInterface;
import cn.facebook.excelUtil.ExcelDataFormatException;
import cn.facebook.excelUtil.MatchupData;
import cn.facebook.excelUtil.SimpleExcelUtil;
import cn.facebook.service.creditor.ICreditorService;
import cn.facebook.util.constant.ClaimsType;
import cn.facebook.utils.ConstantUtil;
import cn.facebook.utils.DateUtil;
import cn.facebook.utils.FrontStatusConstants;
import cn.facebook.utils.RandomNumberUtil;
import cn.facebook.utils.RegValidationUtil;
...
public class MultipleCreditorAction extends BaseAction{
	...
	public void upload() {
		// 1.上传
		String path = this.getRequest().getSession().getServletContext().getRealPath("/WEB-INF/uploadExcel");//上传到服务器的地址
		String destName = new Date().getTime() + fileFileName; //文件名
		File destFile = new File(path, destName); //目标文件
		InputStream is = null;
		try {
			FileUtils.copyFile(file, destFile); //文件上传操作
			
			// 2.读取上传文件的内容，解析它得到List<CreditorModel>
	...
	public void download(){
		...
		try {
			...
			fis = new FileInputStream(path);//文件输入流
			String mimetype = this.getRequest().getSession().getServletContext().getMimeType("ClaimsBatchImportTemplate.xlsx");
			String filename = DateUtil.getDateStr(new Date(), "yyyyMMddHHmmss")+path.substring(path.lastIndexOf("."));//文件名
