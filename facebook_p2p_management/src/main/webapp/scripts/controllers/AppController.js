		...
		.controller(
				...
				function($rootScope, $scope, $state, AdminService) { //将AdminService注入到了当前的loginCtrl中。
					...
					$scope.login = function(username, password) {
						...
						//$http.post(option.url.api_account + 'login',$str, option.header)
						//$http.post(/facebook_p2p_action/account/login,$str, option.header).success(function(response);)
		...
		.controller(
				...
				function($scope, AuthService, FileUploader, hmd) {
					...
					var uploader = $scope.uploader = new FileUploader({
						url : '/facebook_p2p_action/creditor/upload'
					});
						...
						PostService.checkCredit(str).success(
								function(response) {
									if (response.status == 1) {
										...
										var datasum = response.data.datasum;// 合计数据
